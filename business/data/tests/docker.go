...
//Container tracks information about the docker container started for the tests.
...
func startContainer(t *testing.T, image string, port string) *Container {
	name := "postgres"
	envPwd := "POSTGRES_PASSWORD=postgres"
	cmd := exec.Command("docker", "run", "-P", "--name", name, "-e", envPwd, "-d", image)
	var out bytes.Buffer
	cmd.Stdout = &out
	if err := cmd.Run(); err != nil {
		t.Fatalf("could not start container %s: %v\n", image, err)
	}

	id := out.String()[:12]

	cmd = exec.Command("docker", "inspect", id)
	out.Reset() //Clear the buffer of previous command's output
	cmd.Stdout = &out
	if err := cmd.Run(); err != nil {
		t.Fatalf("could not inspect container %s: %v\n", id, err)
	}
...
func extractIPPort(t *testing.T, doc []map[string]interface{}, port string) (string, string) {
	...
	if !exists {
		t.Fatal("could not get network port/tcp settings")
	}
	...
	if len(list) != 1 {
		t.Fatal("could not get network port/tcp list settings")
	}
...
func stopContainer(t *testing.T, id string) {
	if err := exec.Command("docker", "stop", id).Run(); err != nil {
		t.Fatalf("could not stop container: %v\n", err)
	}
	t.Log("Stopped:", id)
